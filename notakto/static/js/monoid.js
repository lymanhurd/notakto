/**
 * Created by lhurd on 2/20/17.
 */

var monoidModule = (function () {
    var my = {};

    my.isWinning = function (boards) {
        var value = findComposite(boards);
        for (var i = 0; i < winningIndices.length; i++) {
            if (value == winningIndices[i]) {
                return true;
            }
        }
        return false;
    ...
    function findComposite(boards) {
        var value = 0; /* index 0 = identity */
        for (var i = 0; i < boards.length; i++) {
            value = multiply(value, boardValue(boards[i]));
        }
        return value;
    }

    function multiply(m1, m2) {
        return multiplicationTable[Math.min(m1, m2) + '*' +
        Math.max(m1, m2)];
    }
